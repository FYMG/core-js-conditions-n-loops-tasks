...
function convertNumberToString(numberStr) {
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    ...
    if (numberStr.length - 1 !== i) result += ' ';
  }
  return result;
}
...
function isPalindrome(str) {
  let reverseStr = '';
  for (let i = 0; i < str.length; i += 1) reverseStr = str[i] + reverseStr;
...
function getBalanceIndex(arr) {
  let arrSum = 0;
  let leftSum = 0;
  for (let i = 0; i < arr.length; i += 1) arrSum += arr[i];
...
function getSpiralMatrix(size) {
  ...
  for (
    let paddingTL = 0, paddingRB = size - 1;
    paddingTL <= paddingRB;
    paddingRB -= 1
  ) {
    for (; y < paddingRB; y += 1) {
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) newMatrix[i] = [];

  for (let col = 0; col < matrix.length; col += 1) {
    for (let row = 0; row < matrix.length; row += 1) {
      newMatrix[col][row] = matrix[matrix.length - row - 1][col];
    }
  }

  for (let col = 0; col < matrix.length; col += 1) {
    for (let row = 0; row < matrix.length; row += 1) {
      result[col][row] = newMatrix[col][row];
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  while (iterationsLeft > 0) {
    ...
    for (let i = 0; i < result.length; i += 1) {
      if (i % 2 !== 0) oddStr += result[i];
      else evenStr += result[i];
    }
...
function getNearestBigger(number) {
  ...
  while (i >= 0 && numArray[i] >= numArray[i + 1]) i -= 1;
  while (numArray[j] <= numArray[i]) j -= 1;

  if (i === -1) return number;
  [numArray[i], numArray[j]] = [numArray[j], numArray[i]];

  numArray.push(...numArray.splice(i + 1).reverse());
